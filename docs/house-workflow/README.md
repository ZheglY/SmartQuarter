...
«фото → проблема → подтверждения → заявление → статус». Актуальные роли и
сценарии описаны в [руководстве продукта](../product-workflows.md).
...
2. Соберите и запустите приложение. Первый вход через MAX создаёт пользователя,
   но **не выдаёт роль или дом**. Гражданин видит поиск домов; регистрация доступна назначенному председателю.
...
6. Администратор открывает «Профиль → Администрирование платформы» и назначает
   пользователя председателем. Председатель создаёт заявку на регистрацию дома;
   администратор проверяет адрес и полномочия, затем одобряет.
