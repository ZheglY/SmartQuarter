...
Дополнение: [регистрация домов, ADMIN_USER_IDS, миграции и сценарии работы](../house-workflow/README.md).
Обычное подключение дома происходит через заявку и одобрение; `/app provision` — инструмент оператора.
## Развёртывание на удалённом сервере
Ubuntu 24.04 LTS | Docker Compose | HTTPS | MAX

Практическое руководство для SmartQuarter. Обновлено 26 сентября 2026 года: назначение председателей, регистрация домов, опросы, календарь и инициативы.
...
**Интеграция:** Gateway подключён к настоящему Identity по общему gRPC-контракту. Вход MAX создаёт сессию, доступ к дому определяется ACTIVE membership. Администратор назначает председателей в Mini App; председатель принимает заявки жильцов. Перед приёмкой нужны ваши домен, токен MAX, S3 и проверенные MAX ID.
...
Проверьте изменения и тесты, затем опубликуйте commit в ветке feat/house-identity-service. Для обычной установки после слияния используйте main. Сервер должен получать именно проверенный commit; локальные незакоммиченные файлы через git pull не передаются.

```bash
git status
git diff --check
git diff --stat
# После commit проверенных изменений:
git push -u origin feat/house-identity-service
```
...
| MAX_CA_BUNDLE | Путь к доверенному CA bundle на сервере; см. раздел 8 |
| ADMIN_USER_IDS | Внутренние UUID администраторов через запятую; см. раздел 9 |
...
- community-migrate выполняет SQL 000001, 000002 и 000003 через psql с ON_ERROR_STOP.
...
При ошибке x509 у Bot API проверьте актуальную цепочку доверия, включая требования MAX к сертификату Минцифры [3]. Установите сертификаты из официального источника https://www.gosuslugi.ru/landing/crt в /usr/local/share/ca-certificates на Ubuntu и выполните sudo update-ca-certificates. В deploy/server/.env задайте MAX_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt. Compose монтирует этот файл в Gateway только для чтения; после изменения пути пересоздайте Gateway. Для scratch-образа пакетный менеджер внутри контейнера недоступен. Не используйте curl -k и не отключайте TLS-проверку в Go.
...
Новый пользователь входит через MAX без дома. Первый вход не выдаёт роль администратора. Оператор проверяет MAX ID будущего администратора и находит внутренний UUID:

```bash
sudo docker compose exec identity-db psql -U identity -d identity_db
```

```sql
SELECT id, max_user_id, display_name FROM users
WHERE max_user_id = 123456789;
```

Подставьте проверенный MAX ID вместо примера. В deploy/server/.env запишите полученный UUID в ADMIN_USER_IDS; несколько UUID разделяются запятыми. Примените конфигурацию:

```bash
sudo docker compose up -d --force-recreate identity-service
```

Администратор открывает «Профиль → Администрирование платформы», находит будущего председателя и нажимает «Назначить председателем». Пользователь должен хотя бы раз войти через MAX. Председатель подаёт заявку через «Дома → Зарегистрировать дом». Администратор рассматривает её в разделе регистраций. После одобрения создаются дом и активное председательство.

Жители находят дом через поиск или приглашение и отправляют заявки. Председатель принимает их в разделе заявок жильцов. Принятый житель выбирает дом и получает доступ к данным.

Для существующего дома администратор выбирает пользователя и назначает его в карточке дома. Прежний председатель становится жителем. «Снять с этого дома» снимает конкретное назначение. «Снять полномочия» снимает председательство во всех домах и отменяет ожидающие регистрации и передачи роли. Доступ жителя сохраняется. Роль ADMIN в membership относится только к дому и не даёт доступа к платформенной админке.

Передача полномочий доступна активному жителю, которого администратор уже назначил кандидатом в председатели. Получатель должен подтвердить передачу в течение 48 часов. Команда /app provision остаётся закрытым инструментом оператора для восстановления доступа; обычное подключение выполняется через интерфейс.
...
Проверьте также истечение сессии, другой дом, неверный Origin, S3 CORS, клавиатуру, BackButton и копирование на MAX Android/iOS. Скачивание .txt в нативном MAX сейчас ограничено отсутствием серверного HTTPS file endpoint; используйте копирование. Проверьте раздел «Сообщество»: создание и закрытие опроса, единственный голос жителя, результаты; создание, изменение и удаление события; публикацию инициативы, однократную поддержку и завершение председателем. На внутренних экранах должны оставаться кнопка «Назад» и нижняя навигация.
...
Перед обновлением сохраните текущий SHA, образы и backup БД. Identity автоматически применяет миграцию 000003_chairman_permissions; существующие активные председатели сохранят возможность регистрации домов. Community требует запуска community-migrate, включая 000003_community_integrity. Если проверка связи голоса с вариантом не проходит, миграция остановится без удаления данных: выясните происхождение некорректных записей перед продолжением.

Перед технической сдачей выполните python tools/data-api/validate_data_api.py DATA-API.yaml --openapi contracts/openapi/openapi.yaml из корня. Файл содержит обязательные HTTP-проверки; сессии проверяющим передаются отдельно, по ролям.

Базы меняются независимо от приложения: возврат к старому image не отменяет миграции. Для breaking schema change нужен отдельный план совместимости или восстановление в новые volumes.
...
Этот документ описывает подготовку и порядок развёртывания. Успешные локальные тесты не подтверждают обновление конкретного VPS: после применения новой версии выполните приёмку на своём домене и внутри MAX по разделу 10.
