...
## Текущая реализация: регистрация домов

Identity владеет регистрациями, вступлением, приглашениями, передачей роли,
настройками уведомлений, аудитом и outbox. Новый `HouseService` добавлен к прежнему
protobuf без изменения старых номеров полей. Транзакции и проверки находятся в
`services/identity-service/internal/house`; типизированные RPC и REST адаптеры
генерируются из реестра операций. Community владеет контактами служб в собственном
`internal/contacts` и отдельной SQL-миграции; межсервисных SQL/FK нет.

Gateway формирует actor context из Redis-сессии, публикует REST и доставляет события
Identity/Issue/Community через MAX. Mini App содержит onboarding без членства и
интерфейсы председателя. Доставка at-least-once защищена дедупликацией события и получателя.
[Состояния, права, события и границы MVP](house-workflow/README.md).

## Историческая схема первоначального каркаса

Ниже сохранено исходное сопоставление документов; упоминания «будущего» кода
относятся к первоначальному каркасу, а не к текущему состоянию сервисов.
