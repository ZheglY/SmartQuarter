...
Регистрация домов и вступление доступны без членства. `/houses` объединяет поиск,
заявки и управление; `/chairman/*` содержит приглашения, жильцов, передачу роли и
справочник служб. `/notifications/settings` управляет доставкой через бота.
Все новые ответы проверяются Zod, запросы разделены по пользователю и активному дому.
[Настройка администратора и ручная приёмка MAX](../../docs/house-workflow/README.md).
