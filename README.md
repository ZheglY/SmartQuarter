...
MAX Mini App для жителей и председателя дома. Регистрация дома с одобрением
администратора, вступление жильцов, приглашения, передача председательства,
справочник служб и уведомления дополняют процесс «фото → проблема → подтверждения →
заявление → ручная отправка председателем → изменение статуса».

Новая функциональность и запуск: [регистрация домов и управление](docs/house-workflow/README.md).
...
получает сессию без доступа к дому и может подать заявку. Первого председателя
назначает одобрение администратора платформы (`ADMIN_USER_IDS`); жильцов принимает
председатель. `identity-service /app provision` остаётся командой восстановления оператора.
